...
"""
FastAPI backend for REAL File Search RAG (Option A: delete local temp file after indexing).
...
HARDCODED_API_KEY = "AIzaSyDFbInSRmM99luN2BBxaotgm2tHvRjXizo"  # Replace with your actual API key
# ----------------------------------------

app = FastAPI(title="File Search RAG API (Option A)")
...
def init_client():
    """Initialize google genai client with hardcoded API key. Raises on missing SDK or invalid key."""
    if genai is None:
        raise RuntimeError("google-genai SDK is not installed on the server.")
    try:
        return genai.Client(api_key=HARDCODED_API_KEY)
...
def rest_list_documents_for_store(file_search_store_name: str):
    """
    Call REST to list documents for a File Search store.
...
# =====================================================
# CREATE STORE (creates a File Search store)
# =====================================================
@app.post("/stores/create")
def create_store(payload: CreateStoreRequest):
    """
    Create file search store. Uses hardcoded API key.
    Response contains:
    ...
    try:
        ...
        return JSONResponse({"error": f"Failed to create File Search store: {e}"}, status_code=500)
...
async def upload_files(
    ...
    Upload and index files to the given store. Returns:
      - filename, uploaded, indexed (bool), document_id
    """
    data = load_data()
    ...
    for upload in files:
        ...
        # ---- upload & index directly into File Search (this chunks + embeds + indexes) ----
        document_resource = None
        document_id = None
        indexed_ok = False

        try:
            # Use the sanitized filename in the display_name sent to API
            ...
            if not document_resource:
                docs = rest_list_documents_for_store(fs_store_name)
        ...
        results.append({
            ...
            "uploaded": True,
            "indexed": indexed_ok,
            "document_id": document_id  # Return only document_id for deletion purposes
        })
...
@app.get("/stores")
def list_stores():
    """List local stores and files. Uses hardcoded API key."""
    try:
        init_client()
    except Exception as e:
        return JSONResponse({"error": str(e)}, status_code=400)
    ...
    # Clean response to remove sensitive information but keep document_id for deletion
    clean_stores = []
    for store in data["file_stores"].values():
        ...
        # Add file information with document_id for deletion but without other sensitive data
        for file_info in store.get("files", []):
            clean_file = {
                "display_name": file_info["display_name"],
                "size_bytes": file_info["size_bytes"],
                "uploaded_at": file_info["uploaded_at"],
                "indexed": file_info["indexed"],
                "document_id": file_info.get("document_id")  # Keep document_id for deletion
            }
            clean_store["files"].append(clean_file)
            
        clean_stores.append(clean_store)
    
    return {"success": True, "stores": clean_stores}

# =====================================================
# DELETE DOCUMENT (calls REST to remove doc from File Search store)
# =====================================================
@app.delete("/stores/{store_name}/documents/{document_id}")
def delete_document(store_name: str, document_id: str):
    """
    ...
    params = {"force": "true", "key": HARDCODED_API_KEY}
    ...
    if resp.status_code not in (200, 204):
        return JSONResponse({"success": False, "error": f"REST DELETE failed: {resp.text}"}, status_code=resp.status_code)
...
def delete_store(store_name: str):
    ...
    # Attempt delete on API (best-effort)
    try:
        client = init_client()
...
# =====================================================
# ASK QUESTION (RAG) - uses File Search tool
# =====================================================
@app.post("/ask")
def ask_question(payload: AskRequest):
    ...
    try:
        ...
        return {"success": True, "response_text": getattr(response, "text", "")}
